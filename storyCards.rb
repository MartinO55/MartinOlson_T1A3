...
#TODO - holds the list of all the cards
    ...
    #The fall does 1d-1 cr damage, with only soft armor protecting.
           # getNextCard(cardDestinations)
            ...
            if Player.hitpoints <=0
                puts "you fall and hit your head. Your unconcious body is later devoured by flesh eating ghouls"
            elsif card5Menu ==1
                Card3.new.startCard3
            else 
            ...
            card7ToPass = Player.dexterity
                if card7Test > card7ToPass #fail
                  #  darkness = true
                    ...
                    puts"you keep hold of your torch"
                   # darkness = false
                end
 ...
 class Card11 <StoryCard
    ...
    #  puts "[2] Try forcing the door open"
        ...
        menu.choice "[1] Use your dagger to pick the lock", 1
        menu.choice "[2] Try forcing the door open",2 
        end 
...
class Card12 <StoryCard
    def startCard12()
        ...
        card12menu = prompt.select ("Which path do you take?") do |menu|
            menu.choice "[1] The trodden path", 1
            menu.choice "[2] The icier path",2 
            menu.choice "[3] The warmer, rock path", 3
        end
...
class Card13 <StoryCard
    ...
    card13menu = prompt.select ("...") do |menu|
        menu.choice "[1] Continue (with a new shield)", 1
    end
   # getNextCard(cardDestinations)
        if card13menu ==1
...
class Card14 <StoryCard
    ...
    card14menu = prompt.select("What now?") do |menu|
        menu.choice "[1] Search the area around the corpse",1 
        menu.choice "[2] Continue onwards", 2
    end
    #getNextCard(cardDestinations)
        if card14menu == 1
...
class Card15 <StoryCard
    ...
    #card15Continue = gets.chomp.to_i
        ...
        card15menu = prompt.select ("...") do |menu|
            menu.choice "[1] Climb down the mountain...", 1
        end
...
class Card16 <StoryCard
   ...
   card16Menu = prompt.select ("Now what?") do |menu|
    menu.choice "[1] Try to pry the dagger out", 1
    menu.choice "[2] Leave it alone and keep going",2 
   end
...
class Card17 <StoryCard
    ...
    # puts "[1] Raise your weapons"
        ...
        card17menu = prompt.select ("...") do |menu|
            menu.choice "[1] Raise your weapons",1
        end
...
class Card18 <StoryCard
    ...
    card18menu = prompt.select ("what do you do") do |menu|
        menu.choice "[1] Try to pick the lock", 1
        menu.choice "[2] Try to break the door down",2 
        menu.choice "[3] Try the servants door instead",3
    end
    ...
    def startCard19() 
        ...
        #puts card19ToPass
            if card19test > card19ToPass#fail
                ...
                damageFromCold = damageFromColdmid -3  #also need to do D6 -3 damage here
                Player.hitpoints -= damageFromCold
               
                    if Player.hitpoints <=0
                        puts "you collapse in the snow, dead"
                    else
            ...
            card19menu = prompt.select ("Where to now?") do |menu|
                menu.choice "[1] Circle around the castle to look for an entrance", 1
                menu.choice "[2] Climb the wall with your rope and pitons", 2
            end
...
card20menu = prompt.select ("....") do |menu|
    menu.choice "[1] Prepare to Fight!",1
end
...
class Card21 <StoryCard
    ...
    card21menu = prompt.select ("phheww") do |menu|
            menu.choice "[1] Continue onwards ...", 1
        end
...
class Card22 <StoryCard
    ...
    card22menu = prompt.select ("Which way now?") do |menu|
        menu.choice "[1] Climb down the cliff", 1
        menu.choice "[2] Walk around the wind torn cliff",2 
...
class Card23 <StoryCard
    ...
    card23menu = prompt.select ("...") do |menu|
        menu.choice "[1] Yoink!",1 
        menu.choice "[2] Examine the tapestries more closely",2
    end
...
class Card26 <StoryCard
    ...
    card26menu = prompt.select("What will you do now?") do |menu|
        menu.choice "[1] Search the courtyard",1 
        menu.choice "[2] Go through the servants door", 2
        menu.choice "[3] Go through the Guardhouse door",3 
    end
...
class Card27 <StoryCard
    ...
    puts "You slowly pad into the icy maze, trying to remember the turns you take.\n\nEvery once in a while you hear a soft moan from ahead of you."
        card27menu1 = prompt.select ("...") do |menu|
            menu.choice "[1] Follow the sounds...?", 1
        end
    ...
    card27TestToPass = card27TestMidway -3
        ...
        if card27Test <= card27TestToPass
            ...
            card27menu2 = prompt2.select ("You strain to listen, but are unable to determine where the sound is coming from") do |menu|
                menu.choice "[1] Take the trodden path instead", 1
                menu.choice "[2] take the icy path instead",2 
            end
...
class Card28 <StoryCard
    ...
    card28menu = prompt.select ("What do you do?") do |menu|
        menu.choice "[1] Return to the main corridor and keep going down it", 1
        menu.choice "[2] Draw your weapon and lift the mattress",2
    end
...
class Card29 <StoryCard
    ...
    #You take 1d-2 cr damage from the fall.
        ...
        card29menu = prompt.select ("What will you do now?") do |menu|
            menu.choice "[1] Try the stairs again...", 2
            menu.choice "[2] Forget these cursed stairs. Go down the main Corridor", 2
        end
    # puts "What will you do now?"
...
class Card30 <StoryCard
    ...
    puts "[1] You search the room"#make a perception roll
        card30menu = prompt.select ("[1] You search the room") do |menu|
            menu.choice "Anything nice?", 1
        end
...
class Card31 <StoryCard
    ...
    puts "You silently pad up the trodden path. The cave comes to a sharp turn.\n\nYou peer around the bend and see a hulking, blood-covered ice troll chewing apart a half-dead seal.\n\nHe’s licking his knife-sized claws and seems to be enjoying the pitiful squeals the seal is making."
        card31test = prompt.select ("shhhh") do |menu|
            menu.choice "[1] You make a stealth check", 1
        end
...
class Card32 <StoryCard
    ...
    puts "[3] Leave the cave and trek to the castle through the snow"
        card32menu = prompt.select ("Which way will you go now?") do |menu|
            menu.choice "[1] The trodden Path (even though it goes to the lair of the creature)",1
            menu.choice "[2] The Icier Path",2
            menu.choice "[3] Leave the cave and trek to the castle through the snow",3
        end
   #getNextCard(cardDestinations)
        if card32menu == 1
            Card9.new.startCard9
...
class Card33 <StoryCard
    ...
    card33menu =prompt.select ("What will you do?") do |menu|
        menu.choice "[1] Examine the hole", 1
        menu.choice "[2] Examine the Tapestries",2
    end
...
class Card34 <StoryCard
 ...
 #   puts "[1] Follow the voice"
        card34menu= prompt.select ("Well?") do |menu|
            menu.choice "[1] Follow the voice"
        end
...
class Card35 <StoryCard
    ...
    puts "You remove your rope and grapple and find a nice target – a broken gargoyle near an open window."
        ...
        card35menu = prompt.select ("is this a good idea?") do |menu|
            menu.choice "[1] This cannot possibly go wrong",1
        end
...
class Card36 <StoryCard
    ...
    card36menu = prompt.select ("What will you do") do |menu|
        menu.choice "[1] Circle around and find an entrance",1
        menu.choice "[2] Use your pitons and rope to climb the walls",2
...
class Card37 <StoryCard
    ...
    puts "[1] You try to help him"# make a Physician Check
        card37menu = prompt.select ("That looks nasty.") do |menu|
            menu.choice "[1] You try to help him",1
        end
...
class Card38 <StoryCard
    def startCard38() 
        ...
        #this is where the darkness modifier (-3 to rolls) pays off, and is the only other combat encounter in the game. Not sure if needed
            card38menu = prompt.select ("*Dramatic Music*") do |menu|
                menu.choice "[1] Your time is up, foul creature"
            end
...
class Card39 <StoryCard
    ...
    card39menu = prompt.select ("Where to now?") do |menu|
        menu.choice "[1] The servants door",1
        menu.choice "[2] The guardhouse door",2
    end
...
class Card40 <StoryCard
    def startCard40()
       ...
       # prompt2 = TTY::Prompt.new
        ...
        card40menu = prompt.select ("Very Heavy") do |menu|
            menu.choice "[1] PUSH",1
            menu.choice "[2] On second thought, maybe I'll try the lock",2
            menu.choice "[3] lets just try the servants door",3
        end
            ...
            card40TestToPass =  card40TestMidway -4
                ...
                if card40Test <= card40TestToPass
                    Card30.new.startCard30
                            ...
                            card40menu = prompt.select ("The door refuses to budge") do |menu|
                                menu.choice "[1] try picking the lock",1 
                                menu.choice "[2] Go try the servants door", 2
                            end
...
class Card41 <StoryCard
    ...
    puts "As the ice troll is licking his claws clean, his beady black eyes falls on you.\nHe lets out a roar from his wierd, circular mouth, tosses the mortally wounded seal against the side of the cave, and charges you"
        card41menu = prompt.select ("well?") do |menu|
            menu.choice "[1] Draw your weapon", 1
        end
...
class Card42 <StoryCard
   ...
   # puts "what now"
        card42menu= prompt.select ("what now") do |menu|
            menu.choice "[1] peer into the room first",1
            menu.choice "[2] Quickly scramble in",2 
        end
...
class Card43 <StoryCard
    ...
    puts "You dig around the bases of the various statues, looking for anything of value."
        card43menu= prompt.select ("anything good?") do |menu|
            menu.choice "[1] I search the room",1 
        end
...
class Card44 <StoryCard
    ...
    puts "You try to sneak past the troll"
        card44menu = prompt.select ("be vewy, vewy quiet") do |menu|
            menu.choice "[1] Quiet, Quiet", 1
        end
...
class Card45 <StoryCard
    ...
    puts "You swing your body over the icy sill of the window and leap into the dim room.\nThe white light shines through the window and illuminates the remnants of this old bedroom. A broken bed lies in the corner of the room, its decorative blanket still well preserved in the cold.\n\nA dresser with a shattered mirror sits opposite the bed, and you can almost feel the ghosts of the nobility who once slept here.\nYou look through some of the dressers, and find nothing but old clothes.\n\nNo, wait.\n\nUnderneath a fine blue chemise is a tarnished silver tiara.\n\nSmiling, you place the jewelry and the blue chemisein your sack.\n\nYou walk to the door and peer out.\n\nDark stairs lead upwards into a turret, and to the right, a corridor heads down to more abandoned bedrooms"
        card45menu = prompt.select ("Which way now?") do |menu|
            menu.choice "[1] Up the stairs",1 
            menu.choice "[2] On to another bedroom",2
            menu.choice "[3] Skip the bedrooms, straight down the corridor", 3
        end
...
class Card46 <StoryCard
    ...
    #take 1 damage if you are still alive, keep going
        ...
        if Player.hitpoints <=0
            ...
            card46menu = prompt.select ("What will you do") do |menu|
                menu.choice "[1] bugger this! (run from the castle)", 1
                menu.choice "[2] Relight your torch and kick the door in, weapon in hand",2
            end
...
class Card47 <StoryCard
    ...
    puts "You look up and see a crumbled statue of an armored warrior in front of you.\n\nYou look around... you found a way into the courtyard of the Castle of Madness!\n\nBeyond the graveyard of dozens of broken statues are two doors.\nThe smaller one seems to be a servant’s entrance; the other seems to be a guardhouse door"
        card47menu = prompt.select ("What will you do?") do |menu|
            menu.choice "[1] Search the courtyard",1
            menu.choice "[2] Go through the servants door",2
            menu.choice "[3] Go through the guardhouse door",3
        end
...
class Card48 <StoryCard
    ...
    puts "With an unnatural gurgle, the gruesome thing falls to the ground, dead.\nLighting a new torch to better see the room, you discover that this was once a servant’s chamber. \n\nThere is a framed picture on the wall of what seems to be the servant tending to a a striking, white-haired queen.\nYou’re startled at the realization that the servant in the painting looks like the bloody creature you just fought and killed."
        card48menu = prompt.select ("well thats just disgusting") do |menu|
            menu.choice "[1] You search the room",1 
        end
    ...
    #getNextCard(cardDestinations)
        if card48Test <= card48TestToPass #success
            ...
            card48menu2 = prompt2.select("Yuck!") do |menu|
                menu.choice "[1] Leave this place",1 
            end
...
class Card49 <StoryCard
    def startCard49() 
        ...
        #  puts "[1] DIE FOUL CREATURE!!!!!!!"
            card49menu = prompt.select ("Draw your weapon!") do |menu|
                menu.choice "[1] DIE FOUL CREATURE!!!!!!!", 1
            end
...
class Card50 <StoryCard
    def startCard50() 
        ...
        # make a DX roll (oh thats dexterity. that makes so much sense now)
            card50menu = prompt.select ("careful....") do |menu|
                menu.choice "hold on...", 1
            end
...
class Card51 <StoryCard
    def startCard51() 
        ...
        #check strength, if not 14 or higher, you can only take 1
            card51menu = prompt.select ("Which will you take?") do |menu|
                menu.choice "[1] The Tapestry of the blue-eyed maiden in ornate armor",1
                menu.choice "[2] The Tapestry of the runed axe lying being presented to a white-haired queen"
            end
            ...
            #getNextCard(cardDestinations)
            if card51menu == 1 #but with the relevant one in your inventory
                Card55.new.startCard55 
            else card51menu == 2
                Card55.new.startCard55
            end
...
class Card52<StoryCard
    def startCard52() 
        ...
        card52TestToPass = Player.dexterity
        ...
        puts "The cliff is icier than expected.\nYou slowly grasp and claw your way around the cliff, the winds constantly threatening to pick you up and toss you over.\n\nAs you circle around the cliff, a powerful gust picks up and sends you hurling towards the edge!"
                #make a DX check
                card52menu = prompt.select ("uh oh") do |menu|
                    menu.choice "[1] Grab for the edge!!!", 1
                end
            # puts "[1] Grab for the edge!!!"
             
      
        #getNextCard(cardDestinations)
        if card52Test <= card52TestToPass #succeed
            puts "you just make it"
            timewaster = gets.chomp.to_i
            Card36.new.startCard36
        else card52Test > card52TestToPass #fail
            #Player.hitpoints -=5
            nextcheck=Player.hitpoints
            if nextcheck <= 0
                puts "you hit the ground. your unconscious body is eaten by grave ghouls"
            else
                # card52menu2 = prompt.select ("You hit the ground. Hard.") do |menu|
                #     menu.choice "[1] Continue",1 
                # end
                # timewaster = gets.chomp.to_i
                Card36.new.startCard36
            ...
            # Card36.new.startCard36 #but take 2d6-2 fall damage. 
        end
    end
end
...
class Card53 <StoryCard
    def startCard53() #this needs to clear the screen
        ...
        card53menu = prompt.select ("well this has definitely been a day") do |menu|
            menu.choice "[1] Continue...", 1
        end
    # puts "[1] Continue..."
...
class Card54 <StoryCard
    def startCard54() #this needs to clear the screen
        ...
        card54menu = prompt.select ("What will you do?") do |menu|
            menu.choice "[1] Shout your name and make your presence known", 1
            menu.choice "[2] Sneak up and try to see what is making the noise",2 
        end
...
class Card55 <StoryCard
    def startCard55() 
        ...
        card55TestToPass = Player.dexterity
            if card55Test <= card55TestToPass
                ...
                Player.hitpoints -=1
                    if Player.hitpoints <=0
                        puts "you died, anticlimactically"
                        Card1.new.startCard1
                    else
                        puts "oww"
                    end
            end
           card55menu = prompt.select ("Now what?") do |menu|
                menu.choice "[1] Creep up to the door and listen", 1
                menu.choice "[2] Go forward bravely and push the door aside",2 
           end
    ...
    class Card56 <StoryCard
       ...
       # puts "[1] Time to leave this place"
            card56menu = prompt.select ("nice.") do |menu|
                menu.choice "[1] Time to leave this place", 1
            end
    ...
    class Card57 <StoryCard
       ...
       # puts "[1] Take the Tapestry of the Armoured maiden, and examine the hole in the floor" #add armoured maiden to inventory
            card57menu = prompt.select ("very nice") do |menu|
                menu.choice "[1] Take the Tapestry of the Armoured maiden, and examine the hole in the floor",1 
            end
       # getNextCard(cardDestinations)
            if card57menu == 1
                Card55.new.startCard55
    ...
    class Card58 <StoryCard
        def startCard58() 
            ...
            card58menu = prompt.select ("Keep going") do |menu|
                menu.choice "[1] leave the room and head down the corridor",1 
            end
    ...
    class Card59 <StoryCard
        ...
        puts "Now which way?"
            card59menu = prompt.select ("Now which way?") do |menu|
                menu.choice "[1] Explore the Bedrooms",1 
                menu.choice "[2] Down the main corridor",2 
            end
    ...
    class Card60 < StoryCard
        def startCard60()
           ...
           # puts "Congratulations - You have bested the Castle of Madness, and have quite the tale to tell the folks back in the tavern"
            ...
            card60menu = prompt.select ("Congratulations - You have bested the Castle of Madness, and have quite the tale to tell the folks back in the tavern") do |menu|
                menu.choice "[1] Back to Main Menu",1 
                menu.choice "[2] Quit the game",2 
            end
            #getNextCard(cardDestinations)
                if card60menu ==1 
    ...
    class Card61 <StoryCard
        ...
        puts "“I know,” he answers, grinning as if he heard your thoughts.\n\n“I will not live more than a few hours. My god visited me just a few minutes ago and told me himself!”\n\n“Let me warn you,” he continues.\n\n“The trodden path leads to the home of the creature.\nI went down the icier path at first, but found an impassable cliff. The creature attacked me again there and dragged me to his lair before I escaped The man shudders.\n\n“Here, take this ring and give it to my the guard captain in Winterhaven. He will contact my family and tell them of my useless death.\nAnd if you see that great elk, hunt it down, roast it over a fire... and think of me.”\n\nHe hands you an amethyst ring and you nod.\n\nThe warrior starts shaking and shuddering, and you decide it is better to leave him now.\nYou head back to the slushy path and go back to the main cavern"
            card61menu = prompt.select ("Which way now?") do |menu|
                    menu.choice "[1] Down the trodden path",1
                    menu.choice "[2] The icier path",2
                    menu.choice "[3] Leave the cave and trek to the castle through the snow",3
                end
